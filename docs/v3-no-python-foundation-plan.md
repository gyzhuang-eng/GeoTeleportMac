...
## 0. Historical Handoff Snapshot

**Start with `docs/current-development-status.md` for current work.**
This file is the long-form historical plan and architecture record.

Last updated: **2026-04-28**. Active branch: `v3/windows-host`.
...
**macOS validation is complete.** The product is ready as an unsigned macOS release.
**Windows host packaging has a working baseline.** The primary remaining work is
Windows installer/clean-machine validation plus macOS signing/notarization if
public distribution is required.

### Day-1 verification checklist

```bash
# 1. Correct branch
git rev-parse --abbrev-ref HEAD          # expect: v3/windows-host
...
Phase C and Phase D (macOS validation) are complete. The macOS app is fully
functional as a DMG package containing a bundled Rust helper. Phase F has a
working Windows host package.

Your first tasks are:
1. **Windows installer:** decide MSI/MSIX/EXE and package the current publish folder.
2. **Clean Windows validation:** test on a machine with no developer toolchain.
3. **macOS distribution hardening:** decide whether to add Developer ID signing
   and notarization.
...
**Current platforms.** macOS and Windows both have working host packages backed
by the shared Rust `native-device-core`.

**macOS distribution.** Unsigned `.dmg` / `.app` for now. Users must
intentionally bypass the macOS warning on first launch.

**Windows distribution.** Self-contained publish folder for now. Installer work
is still open.
...
Current scripted release artifacts live under `dist/`:
...
Xcode IDE builds still create cache products under Xcode's standard DerivedData
folder:
...
repeatable scripted packaging, then copies the release app into `dist/macos`
and removes the intermediate `Build/Products/Release/GeoTeleportMacV3.app`.
For normal Xcode IDE development, use Xcode's standard DerivedData.
...
1. **UI selection.** Thin .NET Windows Forms host with WebView UI.
2. **Core integration.** `native-device-core` builds as Windows DLL + CLI and is
   bridged through the C ABI.
3. **Daemon.** Windows host falls back to bundled CLI
   `ios17-location-daemon` for iOS 17+.
4. **Package folder.** `publish_windows_host.ps1` writes to
   `dist/windows/GeoTeleportWindows-win-x64`.

Remaining:

1. **Host USB/driver setup.** Clean-machine validation of Apple Mobile Device
   Support sufficiency vs driver assistant needs.
2. **Installer.** MSIX, MSI, or EXE.
3. **Diagnostics parity.** Bring Windows diagnostics closer to macOS.
...
| F     | Windows port                           | 🟡 PACKAGE BASELINE DONE (installer + clean validation pending) |
